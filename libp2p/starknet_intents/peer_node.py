...
async def send_intent_via_peer(bootstrap_addr: str, intent: dict[str, Any]) -> None:
    """
    Connects to a libp2p network and publishes a provided intent.
    """
    host = new_host()
    ...
    async with host.run(listen_addrs=[Multiaddr("/ip4/0.0.0.0/tcp/0")]):
        ...
        await trio.sleep(1)
        await pub.publish(TOPIC, json.dumps(intent))
        logger.info("[PEER] Intent broadcasted manually")
