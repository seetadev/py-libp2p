...
class QuicStream:
    ...
    def __init__(
        ...
        # Explicitly annotate the send and receive channels
        self._send_channel: MemorySendChannel[bytes]
        self._receive_channel: MemoryReceiveChannel[bytes]
    ...
    async def read(self, n: int = -1) -> bytes:
        ...
        try:
            ...
            logger.debug(
                "stream_data_received", stream_id=self.stream_id, bytes_count=len(data)
            )
            ...
            self._error = e
            logger.error(
                "stream_read_error", stream_id=self.stream_id, error=format_error(e)
            )
            raise QuicStreamError(
                f"Failed to read from stream: {format_error(e)}"
            ) from e
    ...
    async def write(self, data: bytes) -> None:
        ...
        try:
            ...
            logger.debug(
                "stream_data_sent", stream_id=self.stream_id, bytes_count=len(data)
            )
            ...
            self._error = e
            logger.error(
                "stream_write_error", stream_id=self.stream_id, error=format_error(e)
            )
            raise QuicStreamError(
                f"Failed to write to stream: {format_error(e)}"
            ) from e

    @asynccontextmanager
    async def _cleanup_context(self) -> AsyncGenerator[None, None]:
        ...
        if not self._closed:
            async with self._cleanup_context():
                ...
                try:
                    ...
                    raise QuicStreamError(
                        f"Failed to close stream: {format_error(e)}"
                    ) from e
    ...
    async def reset(self) -> None:
        ...
        if not self._reset:
            self._reset = True
            self._closed = True
            self.connection.quic_conn.reset_stream(self.stream_id, error_code=0)
            await self.connection.flush()
...
class QuicRawConnection:
            ...
            if isinstance(event, StreamDataReceived):
                if event.stream_id not in self._streams:
                    self._streams[event.stream_id] = QuicStream(
                        event.stream_id, self, self.remote_peer
                    )
                ...
                logger.info(
                    "handshake_completed", duration=str(self.metrics.handshake_duration)
                )
    ...
    async def _handle_events(
        # task_status=trio.TASK_STATUS_IGNORED,  # type: ignore[attr-defined]
        self,
        ...
        while not self._closed:
            try:
                ...
                self.metrics.errors_count += 1
                logger.error("event_loop_error", error=format_error(e))
                if not isinstance(e, QuicError):
                    raise QuicProtocolError(
                        f"Event loop error: {format_error(e)}"
                    ) from e
                raise

    @asynccontextmanager
    async def _connection_context(self) -> AsyncGenerator[None, None]:
    ...
    async def start(self) -> None:
        ...
        async with trio.open_nursery() as nursery:
            ...
            try:
                ...
                logger.info("connection_started", remote_addr=self.remote_addr)
            except Exception as e:
                logger.error("connection_start_error", error=format_error(e))
                raise QuicConnectionError(
                    f"Failed to start connection: {format_error(e)}"
                ) from e
        ...
        if not self._closed:
            async with self._connection_context():
                ...
                try:
                    ...
                    logger.error("connection_close_error", error=format_error(e))
                    raise QuicConnectionError(
                        f"Failed to close connection: {format_error(e)}"
                    ) from e

    async def flush(self) -> None:
        """Flush pending QUIC packets."""
        data = self.quic_conn.datagrams_to_send(now=trio.current_time())
        for datagram, _addr in data:
            await self._send_datagram(datagram)

    async def _send_datagram(self, datagram: bytes) -> None:
        """Send a QUIC datagram."""
        # You need to implement sending datagrams via a transport, e.g., a Trio socket.
