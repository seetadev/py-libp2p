...
1. Represented in projective (homogeneous) coordinates: (x, y, z) ∋ (x=x/z, y=y/z)
1. Use complete exception-free formulas for addition and doubling
1. Can be decoded/encoded from/to Uint8Array / hex strings using
   `ProjectivePoint.fromHex` and `ProjectivePoint#toRawBytes()`
1. Have `assertValidity()` which checks for being on-curve
1. Have `toAffine()` and `x` / `y` getters which convert to 2d xy affine coordinates
...
1. Represented in extended coordinates: (x, y, z, t) ∋ (x=x/z, y=y/z)
1. Use complete exception-free formulas for addition and doubling
1. Can be decoded/encoded from/to Uint8Array / hex strings using `ExtendedPoint.fromHex` and `ExtendedPoint#toRawBytes()`
1. Have `assertValidity()` which checks for being on-curve
1. Have `toAffine()` and `x` / `y` getters which convert to 2d xy affine coordinates
1. Have `isTorsionFree()`, `clearCofactor()` and `isSmallOrder()` utilities to handle torsions
...
1. [Timing attack](https://en.wikipedia.org/wiki/Timing_attack) considerations:
...
1. `npm install` to install build dependencies like TypeScript
1. `npm run build` to compile TypeScript code
1. `npm run test` will execute all main tests
...
- `sign`
  ...
  - `der` option has been removed. There are 2 options:
    ...
    1. If you must use DER encoding, switch to noble-curves (see above).
- `verify`
