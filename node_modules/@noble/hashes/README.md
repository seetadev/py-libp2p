...
1. The library has been fuzzed by [Guido Vranken's cryptofuzz](https://github.com/guidovranken/cryptofuzz).
...
1. Memory dump considerations: the library shares state buffers between hash
...
1. `npm run test` will execute all main tests. See [our approach to testing](./test/README.md)
1. `npm run test:dos` will test against DoS; by measuring function complexity. **Takes ~20 minutes**
1. `npm run test:big` will execute hashing on 4GB inputs,
